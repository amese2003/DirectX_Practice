...
GameApp::GameApp(HINSTANCE hInstance) : _appInst(hInstance)
{
	CHECK(_app == nullptr);
	_app = this;
}
...
void GameApp::CreateRtvAndDsvDescriptorHeaps()
{
	...
	rtvHeapDesc.NumDescriptors = _swapChainCount;
	rtvHeapDesc.Type = D3D12_DESCRIPTOR_HEAP_TYPE_RTV;
	rtvHeapDesc.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_NONE;
	rtvHeapDesc.NodeMask = 0;
	CHECK(_device->CreateDescriptorHeap(&rtvHeapDesc, IID_PPV_ARGS(_rtvHeap.GetAddressOf())));
	...
	CHECK(_device->CreateDescriptorHeap(&dsvHeapDesc, IID_PPV_ARGS(_dsvHeap.GetAddressOf())));
}

void GameApp::OnResize()
{
	CHECK(_device == nullptr);
	CHECK(_swapChain == nullptr);
	CHECK(_cmdAlloc == nullptr);

	WaitAsync();

	CHECK(_cmdList->Reset(_cmdAlloc.Get(), nullptr));
	...
	_swapChain->ResizeBuffers(_swapChainCount, _clientWidth, _clientHeight, _backBufferFormat, DXGI_SWAP_CHAIN_FLAG_ALLOW_MODE_SWITCH);
	_curBackbuffer = 0;

	CD3DX12_CPU_DESCRIPTOR_HANDLE rtvHandle(_rtvHeap->GetCPUDescriptorHandleForHeapStart());
	for (UINT i = 0; i < _swapChainCount; i++)
	{
		CHECK(_swapChain->GetBuffer(i, IID_PPV_ARGS(&_swapChainBuffer[i])));
		_device->CreateRenderTargetView(_swapChainBuffer[i].Get(), nullptr, rtvHandle);
	...
	depthStencilDesc.SampleDesc.Quality = _4xMsaaState ? (_4xMassQuality - 1) : 0;
...
bool GameApp::InitMainWindow()
{
	...
	_mainWnd = CreateWindow(L"MainWnd", _mainCaption.c_str(),
		WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, width, height, 0, 0, _appInst, 0);
...
bool GameApp::InitDirect3D()
{
	...
	if (hardwareResult < 0)
	{
		...
		CHECK(_dxgiFactory->EnumWarpAdapter(IID_PPV_ARGS(&pWarpAdapter)));
	...
	CHECK(_device->CreateFence(0, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&_fence)));
	...
	CHECK(_device->CheckFeatureSupport(
		D3D12_FEATURE_MULTISAMPLE_QUALITY_LEVELS,
		&msQualityLevels,
		sizeof(msQualityLevels)));

	_4xMassQuality = msQualityLevels.NumQualityLevels;
	assert(_4xMassQuality > 0 && "Unexpected MSAA quality level.");
...
void GameApp::CreateCommandObjects()
{
	...
	CHECK(_device->CreateCommandQueue(&queueDesc, IID_PPV_ARGS(&_cmdQueue)));

	CHECK(_device->CreateCommandAllocator(
		D3D12_COMMAND_LIST_TYPE_DIRECT,
		IID_PPV_ARGS(_cmdAlloc.GetAddressOf())));

	CHECK(_device->CreateCommandList(
		0,
		D3D12_COMMAND_LIST_TYPE_DIRECT,
		_cmdAlloc.Get(), // Associated command allocator
...
void GameApp::CreateSwapChain()
{
	...
	sd.SampleDesc.Quality = _4xMsaaState ? (_4xMassQuality - 1) : 0;
	sd.BufferUsage = DXGI_USAGE_RENDER_TARGET_OUTPUT;
	sd.BufferCount = _swapChainCount;
	...
	CHECK(_dxgiFactory->CreateSwapChain(
		_cmdQueue.Get(),
		&sd,
		_swapChain.GetAddressOf()));
}

void GameApp::WaitAsync()
{
...
void GameApp::CalculateFrameStats()
{
	...
	if (totaltime - timeElapsed >= 1.0f)
	{
		...
		wstring windowText = _mainCaption +
			L"    fps: " + fpsStr +
			L"   Frame Time: " + mspfStr +
			L" (ms)";
